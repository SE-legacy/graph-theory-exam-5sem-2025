...
*Сетью G* = $(V, E)$ называется ориентированный граф, такой, что 
...
*Задача.* Пусть дана сеть $G = (V, E)$. Требуется найти функцию потока:
...
*Величиной* разреза называется сумма пропускных способностей всех дуг, ведущих из множества $A$ в множество $B$.
...
+ На найденном увеличивающем пути пускаем максимально возможный поток.
  + На найденном пути ищем ребро с минимальной пропускной способностью $c_min$.
...
Более эффективной реализацией алгоритма Форда-фалкерсона является алгоритм Эдмондса-Карпа, который вместо обхода в глубину для поиска пути, обходит граф в ширину.
